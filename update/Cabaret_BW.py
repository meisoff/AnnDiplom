...
while time < FullTime:
    ...
    S_cons_half = P_cons_half - C_cell * C_cell * Rho_cons_half
    G_cell = 1 / (C_cell * Rho_cons_half)
    R_speed_cell = U_cons_half + C_cell
    Q_speed_cell = U_cons_half - C_cell
    S_speed_cell = U_cons_half
    R_cons_half = U_cons_half + G_cell * P_cons_half
    Q_cons_half = U_cons_half - G_cell * P_cons_half
    ...
    R_left = U_flux[mask_speed_R] + G_cell[mask_speed_R[:n]] * P_flux[mask_speed_R]
    R_right = np.roll(U_flux, -1)[mask_speed_R] + G_cell[mask_speed_R[:n]] * np.roll(P_flux, -1)[mask_speed_R]
    R_cent = U_cons[mask_speed_R[:n]] + G_cell[mask_speed_R[:n]] * P_cons[mask_speed_R[:n]]
    ...
    R_left2 = np.array(
        np.roll(U_flux, 1)[mask_speed_R2] + np.roll(G_cell, 1)[mask_speed_R2[:n]] * np.roll(P_flux, 1)[mask_speed_R2])

    R_right2 = U_flux[mask_speed_R2] + np.roll(G_cell, 1)[mask_speed_R2[:n]] * P_flux[mask_speed_R2]

    R_cent2 = np.array(
        np.roll(U_cons, 1)[mask_speed_R2[:n]] + np.roll(G_cell, 1)[mask_speed_R2[:n]] *
        np.roll(P_cons, 1)[mask_speed_R2[:n]]
    )
    ...
    # Q приходит справа
    Q_left = U_flux[mask_speed_Q] - G_cell[mask_speed_Q[:n]] * P_flux[mask_speed_Q]
    Q_right = np.roll(U_flux, -1)[mask_speed_Q] - G_cell[mask_speed_Q[:n]] * np.roll(P_flux, -1)[mask_speed_Q]
    Q_cent = U_cons[mask_speed_Q[:n]] - G_cell[mask_speed_Q[:n]] * P_cons[mask_speed_Q[:n]]
